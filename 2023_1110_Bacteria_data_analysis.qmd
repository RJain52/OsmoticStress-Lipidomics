...
mdat$Condition <- factor(mdat$Condition, 
                            levels = c("0.28 OsM", "1.1 OsM", "1.1 OsM + 1mM GB")
                            )
...
Z <- Z |>
  filter(Condition %in% c("0.28 OsM", "1.1 OsM + 1mM GB")  # Change to 1.1 OsM for other volcano plot
         )
...
volcano <- Z |> 
  ...
  summarise(FC = mean(signal[Condition == "1.1 OsM + 1mM GB"])/mean(signal[Condition == "0.28 OsM"])
            ) |>
  ungroup()
...
p0 <- volcano |>
  ...
  annotate("text", label = "p=0.05",
           ...
           y = 1.15,                         # text label for line signifying p-value = 0.05
           size = 2.5,      
           vjust = -0.25,
           colour = "black") +
  ...
  xlab("log2(Fold change [1.1 OsM + 1mM GB/0.28 OsM])") +        # Edit based on groups
  ylab("-log10(p-value)") +
  theme_bw(base_size = 14,
           base_family = "sans",
           base_line_size = 1
...
ggsave(filename="output/volcano_high_GB.pdf", 
       plot = p0, 
       device = "pdf",
       width = 14, height = 16,
       units = "cm",
...
write_csv(volcano, file = "output/volcano_high_GB.csv")

# Use the below code in the scale_color_manual depending on groups being plotted
# This allows for lipid class shading to be similar between plots
########## For 1.1 volcano ##########
...
other attached packages:
 [1] factoextra_1.0.7 ggbreak_0.1.2    rstatix_0.7.2    ggpubr_0.6.0     scales_1.2.1    
